...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 22:38:59 2019
...
def _parse_args(argv):
    ...
    parser.add_argument('--c_th', type=float,
                        default=0.05,
                        help='the cost threshold for stopping training, default = 0.05')
...
def main(argv): 
    ...
    first_folder = 'fold_results_h%d_bs%d_lr%0.5f'%(hidden_num,BATCH_SIZE,lr)
    ...
    with tf.Session(config=config) as sess:
        ...
        while train_cost >cost_threshold:
            ...
            if (i) % 10 ==0:
                ...
                sess.run(initer,feed_dict={X:x_test,batch_size:len(x_test)}) 
                test_cost= sess.run(cost)   
                
                plotdata["epoch"].append(i)
                plotdata["train_cost"].append(train_cost)
                plotdata["test_cost"].append(test_cost)
            ...
            if (i) % 100 ==0:
                ...
                sess.run(inf_initer,feed_dict={X:x_test,batch_size:len(x_test)}) 
                test_rep,test_result_output=sess.run([encode_final_state,decode_output])
                            
    
                plotdata["train_recover_data"]=deepcopy(train_result_output )
                plotdata["test_recover_data"]=deepcopy(test_result_output)
                plotdata["train_representation"]=deepcopy(train_rep)
                plotdata["test_representation"]=deepcopy(test_rep)
                            
                print('Epoch %d'%i,"--train_cost", train_cost,"--test_cost", test_cost)
